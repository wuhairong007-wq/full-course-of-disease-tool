...
assert.match(skill, /trailing test marker|末尾.*测试标识|测试标识.*末尾/s);
assert.match(skill, /16-column|16 columns|16列/);
assert.match(skill, /legacy 17\/18-column|旧17\/18列|17\/18-column/s);
...
assert.match(medicationSchema, /`单次`.*single-use|single-use.*`单次`|单次给药/s);
...
const deepInterviewContract = await fs.readFile(path.join(skillDir, "references", "deep-interview-template-contract.md"), "utf8");
assert.match(deepInterviewContract, /正文主标题固定为“患者访谈分析报告”和“患者访谈记录明细”/);
