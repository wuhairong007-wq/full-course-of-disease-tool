...
def find_header(rows):
    for index, row in enumerate(rows[:8]):
        ...
        candidates = [role for role, required in ROLE_RULES.items() if required.issubset(header_set)]
        if candidates:
...
def build_metrics(data, product):
    ...
    severe_ids = {
        ...
        if clean(record.get("不良反应严重程度分级")) in {"重度", "严重", "危及生命"}
...
def build_insight(source_paths, product, start_text, end_text):
    ...
    return {
        ...
        "metadata": {
            ...
            "contractGoal": "覆盖患者管理全流程，完成健康管理方案、用药提醒、AI智能随访、症状自评和不良反应监测，围绕产品安全性、有效性、依从性和患者体验进行服务评估。",
        },
