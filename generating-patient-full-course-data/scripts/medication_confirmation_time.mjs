...
export function generateMedicationConfirmationTime(patient) {
  ...
  const latestLegalTime = Math.min(activation.getTime() + CONFIRMATION_WINDOW_MS, serviceEnd.getTime() - 1000);
  const firstLegalDate = new Date(earliestLegalTime);
  firstLegalDate.setHours(0, 0, 0, 0);
  const windows = [];
  let totalSeconds = 0;

  for (const date = new Date(firstLegalDate); date.getTime() <= latestLegalTime; date.setDate(date.getDate() + 1)) {
    ...
    const endTime = Math.min(dayEnd.getTime(), latestLegalTime);
  ...
  if (!totalSeconds) throw new Error(`${patient.userid}不存在严格晚于激活时间、在激活时间后7天内且位于服务周期内的合法确认时间（早于服务结束日期，每日07:00:00至21:59:59）`);
