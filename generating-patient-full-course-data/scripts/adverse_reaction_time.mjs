...
export function generateAdverseReactionTime(patient) {
  ...
  if (!totalSeconds) throw new Error(`${patient.userid}当月不存在严格晚于激活时间且位于06:00:00至21:59:59的合法发生时间`);
  const selectedOffset = Math.floor(stableRandom(patient.userid, "adverse-reaction-occurrence-after-activation-06-22") * totalSeconds);
