...
const cases = [
  ["U-DAY", "2026-07-06 19:36:59", "2026-07-31"],
  ["U-EARLY", "2026-07-06 03:20:00", "2026-07-31"],
  ["U-LATE", "2026-07-06 22:30:00", "2026-07-31"],
  ["U-MONTH-END", "2026-07-31 10:09:33", "2026-08-01"],
  ["U-END-SECOND", "2026-07-31 21:59:58", "2026-08-01"],
];

for (const [userid, activationText, serviceEndDate] of cases) {
  const resultText = generateMedicationConfirmationTime({ userid, activateTime: activationText, serviceEndDate });
  ...
  assert.equal(generateMedicationConfirmationTime({ userid, activateTime: activationText, serviceEndDate }), resultText, `${userid}结果不稳定`);
}
...
assert.throws(
  () => generateMedicationConfirmationTime({ userid: "U-NO-WINDOW", activateTime: "2026-07-30 21:59:59", serviceEndDate: "2026-07-31" }),
  /服务周期结束日前不存在严格晚于激活时间且位于06:00:00至21:59:59的合法确认时间/,
);
