...
for (const [userid, activationText] of cases) {
  const resultText = generateMedicationConfirmationTime({ userid, activateTime: activationText });
  ...
  assert.equal(generateMedicationConfirmationTime({ userid, activateTime: activationText }), resultText, `${userid}结果不稳定`);
}

assert.throws(
  () => generateMedicationConfirmationTime({ userid: "U-NO-WINDOW", activateTime: "2026-07-31 21:59:59" }),
  /当月不存在严格晚于激活时间且位于06:00:00至21:59:59的合法确认时间/,
);
