...
- `生成患者明细` must deliver the original 17-column patient workbook built with `assets/patient-full-course-template.xlsx` and `scripts/build_workbook.mjs`. Preserve all patients in source order, the fixed header names/order, exactly one worksheet and one table, the medication/prescription mapping, and `已生成/待确认` statuses. Never substitute a candidate assessment, search report, hypothetical-plan workbook, CSV, Word report, or a redesigned patient layout. Do not add review columns or review sheets to the patient workbook.
...
9. Deliver only the verified 17-column patient workbook. Follow the Final Deliverable Contract for failed or reused runs; internal candidate-review files are not the requested output.
