...
The trigger must include `数量：N`, where `N` is a positive integer. Accept `患者标签` values `无`, `轻度`, `中度`, or `高度`. Select only patients whose label is `轻度`, `中度`, or `高度`; exclude every patient labelled `无`. Preserve source order and select the first `N` eligible patients, with no repeated patient. Stop when fewer than `N` eligible patients exist instead of duplicating records.
...
- `不良反应严重程度分级`: copy `患者标签` exactly (`轻度`, `中度`, or `高度`).
- `是否触发人工干预`: `高度` → `是`; `轻度` and `中度` → `否`.
