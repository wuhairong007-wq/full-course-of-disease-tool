...
Generate patient-detail drafts for review, not executable prescriptions. Use only source disease, age, sex, product, product type, allergy history, actual source symptomEvidence and clinicalContext. Do not invent symptoms, test results, pathology, stage, comorbidities, organ function, weight, contraindications, or efficacy claims. State medication-specific monitoring and safety actions directly; do not add uncertain external-personnel wording such as `由临床医生` or `由临床医师`.

## Source-Supported Role Assessment

Use actual symptomEvidence when present. Preserve negative findings such as `无红肿` and numeric zero; do not assume a pain-score scale or severity cutoff when the source does not specify it; missing symptoms are unknown, not mild, moderate, or severe. `患者标签` is not a pain score or inflammation assessment. Do not fabricate symptoms, use randomized strata, or call the legacy osteoarthritis simulation generator when producing patient details. A request to omit simulation labels does not turn invented symptoms into clinical evidence.

For each patient assess the supplied product, etiologic/first-line treatment, maintenance treatment, perioperative treatment, and symptom-supportive treatment independently. A diagnosis or standard procedure may support a role without an explicit symptom, but a missing symptom alone must never create a new indication. Existing symptoms do not automatically establish a drug choice: review site, severity, negation, contraindications, allergy and duplicate therapy. Report contradictory source findings rather than choosing a convenient value.

When the source or preliminary selection contains only one medication, or the preliminary count is below the requested minimum, complete the active search in `medication-review-schema.md` before concluding the assessment. Search disease-specific combination evidence and official instructions, distinguish add-on therapy from alternatives or escalation, and assess each candidate against actual patient facts. A blank symptom column does not prevent retrieval. Record conditional candidates as unselected until their indication and material safety conditions are supported; do not convert literature recommendations into observed patient facts.

Create a temporary `medication-review.json` array in source order before finalizing prescriptions, following `medication-review-schema.md`. Separate exact original-column evidence from clinical rationale; record candidate indication, safety assessment, eligibility, selection and equivalent groups. Review disease subtype, anatomical site and phase rather than sharing a default regimen across all patients. Use actual clinicalContext for comorbidities, existing medication, treatment response and safety findings; never interpret an empty field as normal. Every final medication must appear as selected in this review, and every selected candidate must appear exactly once in the final medication list. Pass the review to the workbook builder with `--review`; structural checks do not prove clinical correctness.

Generate 3～5 distinct medications when supported by the completed role assessment. If only one or two are supported, keep that smaller set. Do not omit a clearly indicated medication merely because the source lists only one product. Do not invent inflammation, pain, comorbidities or test results, combine equivalent alternatives, or add an unrelated supportive drug just to reach three. There is no minimum of three drugs. Record why a same-disease cohort legitimately has identical treatment roles; stable equivalent selection varies candidates only within an established role.

Every completed record must contain 1–5 clinically justified medications. When `产品类型=用药`, treat the supplied `产品名称` as a source-reviewed medication and include it first, except when `公司=山东利赛医药有限公司`, where the medicinal product is excluded from both generated medication fields. Then independently evaluate first-line disease treatment, maintenance or mandatory postoperative therapy, and every directly indicated adjunct from disease, procedure, age, sex, allergy history, product, reviewed surgery, and actual source symptoms. Finally evaluate each 有明确依据的对症支持药物. Retain every safe, directly indicated treatment role, subject to the five-drug cap. Clinical need determines the count: a disease routinely managed with monotherapy or dual therapy, including uncomplicated vulvovaginal candidiasis and other clinically supportable local vaginal infection regimens, may use one or two medications. Do not default every patient to the same number, randomize the number, or add an unrelated or contraindicated drug merely to reach an arbitrary count. If no safe, directly indicated medication remains after the complete assessment, stop and report the affected `userid` instead of writing an empty or padded record.

## Symptom-Supportive Medication Gate

Add a symptom-supportive medication only when actual source symptoms, the named disease, standard procedure, or already selected treatment establishes a recognized indication without inventing a symptom. Examples include quantified short-course non-opioid analgesia after an operation, sputum clearance after a procedure that impairs airway clearance, and bowel management when the supplied operation itself creates a standard postoperative constipation risk. Infection treatment still requires explicit infectious evidence in the diagnosis or procedure context.
...
Apply the allergy gate to both source-reviewed and AI-selected medications. A medicinal `产品名称` does not override allergy safety: if it conflicts with the source allergy history, stop and report the `userid`, allergy, and product, except in the target company exclusion where the product is omitted entirely. After finalizing `combinedMedication`, verify that every active prescription entry starts with the same non-conflicting medication in the same order. When an allergy changes an AI-selected medication, regenerate the full prescription entry for a clinically equivalent non-conflicting option and state `因XX过敏，改用XX` or a precise equivalent. Never mention a conflicting drug as an active instruction in `处方清单`.

## Surgery

- `产品类型=器械`: provide only the standard procedure name matching the disease and device use. Never append `（使用<产品名称>）` or another product-use note to `手术名称`. The device is not a medication. Stage 1 writes `耗材名称` from `产品名称` after removing only a trailing test marker such as `-测试`, `（测试）`, `测试用`, or `测试版`; prescription display follows `record-schema.md`.
